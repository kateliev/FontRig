...
FontRig.TF_HANDLE_SIZE = 4;   // half-size in screen pixels
FontRig.TF_ROTATE_MARGIN = 12; // screen pixels outside frame for rotate zone

// -- Handle layout --------------------------------------------------
// 0=TL, 1=TC, 2=TR, 3=MR, 4=BR, 5=BC, 6=BL, 7=ML
FontRig._tfHandlePositions = function() {
	var b = FontRig.tf.bbox;
	if (!b) return [];
...
FontRig.activateTransform = function() {
	...
	FontRig.tf.active = true;
	FontRig.tf.mode = 'scale';
	FontRig.tf.bbox = { minX: minX, minY: minY, maxX: maxX, maxY: maxY };
	FontRig.tf.origin = { x: (minX + maxX) / 2, y: (minY + maxY) / 2 };
	FontRig.tf.startPositions = startPos;
	FontRig.tf.dragType = null;
	FontRig.tf.cumRotation = 0;
	FontRig.tf.cumSkewX = 0;
	FontRig.tf.cumSkewY = 0;
...
FontRig.deactivateTransform = function() {
	FontRig.tf.active = false;
	FontRig.tf.bbox = null;
	FontRig.tf.startPositions = null;
	FontRig.tf.dragType = null;
	FontRig._tfClearStatus();
	// (draw handled by caller)
};

// -- Cycle mode on double-click -------------------------------------
FontRig.cycleTransformMode = function() {
	var modes = ['scale', 'rotate', 'skew'];
	var idx = modes.indexOf(FontRig.tf.mode);
	FontRig.tf.mode = modes[(idx + 1) % modes.length];
	FontRig._tfUpdateStatus();
	// (draw handled by caller)
};
...
FontRig.tfHitTest = function(sx, sy) {
	if (!FontRig.tf.active || !FontRig.tf.startPositions) return { type: null };

	var hs = FontRig.TF_HANDLE_SIZE;

	// Check origin
	var osp = FontRig.glyphToScreen(FontRig.tf.origin.x, FontRig.tf.origin.y);
	...
	// Compute live bbox
	var minX = Infinity, minY = Infinity, maxX = -Infinity, maxY = -Infinity;
	for (var id of FontRig.tf.startPositions.keys()) {
		var ref = FontRig.findNodeById(id);
		if (!ref) continue;
		if (ref.node.x < minX) minX = ref.node.x;
		if (ref.node.y < minY) minY = ref.node.y;
	...
	for (var i = 0; i < handles.length; i++) {
		var sp = FontRig.glyphToScreen(handles[i].x, handles[i].y);
	...
	var rm = FontRig.TF_ROTATE_MARGIN / FontRig.state.zoom;
...
FontRig.drawTransformFrame = function() {
	if (!FontRig.tf.active || !FontRig.tf.startPositions) return;
	...
	for (var i = 0; i < corners.length; i++) {
		var sp = FontRig.glyphToScreen(corners[i].x, corners[i].y);
	...
	for (var i = 0; i < pts.length; i++) {
		var sp = FontRig.glyphToScreen(pts[i].x, pts[i].y);
	...
	var osp = FontRig.glyphToScreen(tf.origin.x, tf.origin.y);
	...
	var tlsp = FontRig.glyphToScreen(minX, maxY);
...
FontRig._tfApplyScale = function(gx, gy) {
	...
	var handles = FontRig._tfHandlePositions();
...
// -- Rotate from drag -----------------------------------------------
FontRig._tfApplyRotate = function(gx, gy) {
	var tf = FontRig.tf;
	var ox = tf.origin.x, oy = tf.origin.y;
...
FontRig._tfApplySkew = function(gx, gy) {
	var tf = FontRig.tf;
	var b = tf.bbox;
	var hi = tf.dragHandle;
	var ox = tf.origin.x, oy = tf.origin.y;
...
FontRig.tfMouseDown = function(sx, sy, e) {
	if (!FontRig.tf.active) return false;

	var hit = FontRig.tfHitTest(sx, sy);
	if (hit.type === null) {
	...
	var gp = FontRig.screenToGlyph(sx, sy);
	FontRig.tf._shiftKey = e.shiftKey;

	if (hit.type === 'origin') {
		FontRig.tf.dragType = 'origin';
		FontRig.tf.dragStart = { gx: gp.x, gy: gp.y };
		return true;
	}

	if (hit.type === 'handle') {
		FontRig.tf.dragType = 'handle';
		FontRig.tf.dragHandle = hit.idx;
		FontRig.tf.dragStart = { gx: gp.x, gy: gp.y };
		return true;
	}

	if (hit.type === 'rotate' && FontRig.tf.mode === 'rotate') {
		FontRig.tf.dragType = 'rotate';
		FontRig.tf.dragStart = { gx: gp.x, gy: gp.y };
		return true;
	}
...
FontRig.tfMouseMove = function(sx, sy, e) {
	if (!FontRig.tf.active || !FontRig.tf.dragType) return false;

	var gp = FontRig.screenToGlyph(sx, sy);
	FontRig.tf._shiftKey = e.shiftKey;

	if (FontRig.tf.dragType === 'origin') {
		FontRig.tf.origin.x = Math.round(gp.x);
		FontRig.tf.origin.y = Math.round(gp.y);
		// (draw handled by caller)
		return true;
	}

	if (FontRig.tf.dragType === 'handle') {
		if (FontRig.tf.mode === 'scale') {
			FontRig._tfApplyScale(gp.x, gp.y);
		} else if (FontRig.tf.mode === 'skew') {
			FontRig._tfApplySkew(gp.x, gp.y);
		} else if (FontRig.tf.mode === 'rotate') {
			FontRig._tfApplyRotate(gp.x, gp.y);
		}
		// (draw handled by caller)
		return true;
	}

	if (FontRig.tf.dragType === 'rotate') {
		FontRig._tfApplyRotate(gp.x, gp.y);
		// (draw handled by caller)
		return true;
	}
...
FontRig.tfMouseUp = function() {
	if (!FontRig.tf.active) return false;
	if (!FontRig.tf.dragType) return false;
...
FontRig.tfDblClick = function(sx, sy) {
	if (!FontRig.tf.active) return false;

	var hit = FontRig.tfHitTest(sx, sy);
	if (hit.type === 'inside' || hit.type === 'handle' || hit.type === 'rotate') {
		FontRig.cycleTransformMode();
		return true;
	}
	return false;
};
...
FontRig._tfRecalcBbox = function() {
	var minX = Infinity, minY = Infinity, maxX = -Infinity, maxY = -Infinity;
	...
	FontRig.tf.bbox = { minX: minX, minY: minY, maxX: maxX, maxY: maxY };
	FontRig.tf.startPositions = newStart;
	FontRig.tf.cumRotation = 0;
	FontRig.tf.cumSkewX = 0;
	FontRig.tf.cumSkewY = 0;
};
...
FontRig._tfUpdateStatus = function() {
	...
	document.getElementById('tf-r').value = Math.round(FontRig.tf.cumRotation * 10) / 10;
	document.getElementById('tf-sx').value = Math.round(FontRig.tf.cumSkewX * 10) / 10;
	document.getElementById('tf-sy').value = Math.round(FontRig.tf.cumSkewY * 10) / 10;

	var modeEl = document.getElementById('tf-mode');
	if (modeEl) modeEl.textContent = FontRig.tf.mode.charAt(0).toUpperCase() + FontRig.tf.mode.slice(1);
};

FontRig._tfClearStatus = function() {
	var el = document.getElementById('tf-controls');
	if (el) el.style.display = 'none';
};

// -- Numeric input handlers -----------------------------------------
FontRig._tfApplyNumeric = function(field, value) {
	if (!FontRig.tf.active || !FontRig.tf.startPositions) return;
	...
	if (field === 'w' && Math.abs(bw) > 0.1) {
		var sx = value / bw;
		FontRig._tfScaleAll(sx, 1);
	} else if (field === 'h' && Math.abs(bh) > 0.1) {
		var sy = value / bh;
		FontRig._tfScaleAll(1, sy);
	} else if (field === 'r') {
		var angle = value * Math.PI / 180;
		FontRig._tfRotateAll(angle);
	} else if (field === 'sx') {
		var rad = value * Math.PI / 180;
		FontRig._tfSkewAll(rad, 0);
	} else if (field === 'sy') {
		var rad = value * Math.PI / 180;
		FontRig._tfSkewAll(0, rad);
	}
...
// -- Wire numeric inputs --------------------------------------------
FontRig.wireTransformInputs = function() {
	...
	for (var i = 0; i < fields.length; i++) {
		(function(f) {
			...
			el.addEventListener('change', function() {
				...
				if (isNaN(val)) return;
				FontRig._tfApplyNumeric(f, val);
			});
			el.addEventListener('keydown', function(e) {
				if (e.key === 'Enter') {
					e.preventDefault();
					var val = parseFloat(this.value);
					if (!isNaN(val)) FontRig._tfApplyNumeric(f, val);
					this.blur();
				}
				if (e.key === 'Escape') {
					this.blur();
